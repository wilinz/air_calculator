...
enum AircalcNative {
    ...
    /// 按文件路径初始化。裸 Core ML 只能这么来——它没有从字节建模型的入口，
    /// 而 .mlpackage 本身是个目录。
    typealias HandInitPath = @convention(c) (
    ...
    /// 老版本核心库里没有这个符号，取不到就退回 handInit 那条字节路径。
    static let handInitPath: HandInitPath? = symbol("hand_track_init_path")
